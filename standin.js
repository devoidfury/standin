...
function deepParentGetter(obj, fullkey) {
	let splitIdx = fullkey.lastIndexOf('.')
	if (splitIdx === -1) return [obj, fullkey]
	let key = fullkey.substr(0, splitIdx)
	...
	for (let i = 0; i < changes.length; i++) {
		...
		for (let key of currentchanges) {
			...
			for (binding of binds)
				BIND_TYPES[binding.type].handle(binding.el, this, key)
		}
	}
...
CTX.prototype.add_binding = function(el, deepkey, type, selector) {
	let binds = this.bindings
	if (!binds[deepkey])
		binds[deepkey] = []
	if (BIND_TYPES[type].init && !binds[deepkey].some(b => b.type === type)) 
		BIND_TYPES[type].init(selector, this, deepkey)
	binds[deepkey].push({el: el, type: type})
}
...
CTX.prototype.auto = function() {
	Object.keys(BIND_TYPES).forEach(type => {
		...
		let selector = '['+datatype_attr+']';
		[].map.call(this.root.querySelectorAll(selector), el => {
			this.add_binding(el,
...
function on(el, name, selector, fn) {
	el.addEventListener(name, function(e) {
		if (e.target.matches(selector)) return fn.call(e.target, e)
	})
}
...
const BIND_TYPES = {
	...
	'bind-text': {
		...
		init: function(selector, ctx, key) {
			ctx.on('keyup', selector, function(e) {
				let access = deepParentGetter(ctx.data, key)
